...
`iso/build.sh` builds a custom Arch live ISO with this installer baked in. It also builds a signed-at-install rescue UKI artifact that the installer copies to `/efi/EFI/Linux/arch-rescue.efi`. The live environment boots with the repo at `/root/arch-new-install`, `gum`, `impala`, `git`, `btrfs-progs`, `snapper`, and `rsync` preinstalled, and a login message pointing at two launcher commands:
...
During the build, `iso/build.sh` prompts for a temporary rescue root password, builds `artifacts/arch-rescue.efi`, removes the temporary password hash, and embeds the rescue UKI artifact into the ISO. The installed system gets an additional `arch-rescue` EFI boot entry; no `systemd-boot` bootloader is used.
...
- `artifacts/arch-rescue.efi` — generated by `rescue-uki/build.sh` and copied explicitly into the ISO because `artifacts/` is gitignored
